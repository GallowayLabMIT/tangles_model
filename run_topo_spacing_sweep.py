...
def run_sweep(dx_in_bp, topo_multiplier, n_runs, ptype, id):
    ...
    result_pd.to_feather(f'output/dataframes/dx-{dx_in_bp}_topo-{topo_multiplier}_id-{id}_sc={ptype!=ssc.PromoterType.STATIC}.feather')
    print(result_pd)

if __name__ == '__main__':
    pool = ProcessPool()
    pool.map(lambda args: run_sweep(args[0], args[1], 100, args[2], args[3]), itertools.product(

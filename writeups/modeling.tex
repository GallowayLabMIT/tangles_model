...
\begin{figure}
    ...
    \caption{Supercoiling-dependent polymerase motion and polymerase initiation predict context-dependent circuit behavior. All plotted reporter outputs are normalized to the steady-state value of the tandem reporter at zero-induction of the adjacent gene.
        \subref{fig:supercoiling_with_energy_cartoon}  Polymerases generate positive and negative supercoiling ahead and behind as they move. Polymerase initiation can be modeled as independent or dependent of the local supercoiling density. A supercoiling-dependent polymerase initiation model can be further enhanced with a term harmonic with respect to the supercoiling density. % TODO: increase height of row 1
        \subref{fig:reporter_output_by_init_type} Reporter output normalized to the uninduced case is shown as a function of the fold induction of the inducible gene, relative to the reporter gene, for the three different polymerase initiation models.
        \subref{fig:reporter_output_by_alpha} Reporter mRNA counts are shown for the one-fold adjacent induction case as the value of \(\alpha\) is varied. The selected value of \(\alpha = 0.025\) \textit{(black line)} is chosen to occur at the transition between two regimes of behavior.
        \subref{fig:output_distribution_by_orientation} Density plots of the reporter gene and inducible gene outputs are shown in the linear boundary-condition, equally-induced (1-fold) case. Supercoiling-dependence causes circuit syntax to dramatically affect the intrinsic to extrinsic noise ratio.
        ...
        \subref{fig:reporter_output_by_spacing_fold_induction} Deviations in reporter output relative to the maximum intergene-spacing is shown as a function of intergene-spacing. The convergent orientation is most strongly affected by inter-gene spacing. % Axis in kb, not bp
...
To understand the mechanisms that support syntax-specific expression profiles \cref{fig:top:orientation_bc_behavior}, we examined the ensemble supercoiling density and burst behaviors of our two-gene systems. Putatively, differences in supercoiling across the two-gene systems give rise to differences in RNAP initiation and thus impact gene expression. To observe supercoiling across the two-gene systems, we averaged the supercoiling density across the simulated ensemble. To examine how induction of the adjacent gene changes supercoiling density, we compared the profiles for when the adjacent gene is uninduced (zero-fold induction) and induced (one-fold induction). As expected, the uninduced cases uniformly showed that positive supercoiling accumulates upstream of the constitutively-active reporter gene while negative supercoiling accumulates downstream(\cref{fig:base_model_sc_density},\cref{fig:supp:sc_density_induction}). Upon induction, supercoiling accumulates within the intergenic regions in a syntax-specific manner. %TO DO: Add this section here? Put back where it was and make a supplemental figure for circular-linear comparison? "The similarity between linear and circulation boundary conditions (shown above, ref fig) may be explained by examining how supercoiling accumulates. For example, with linear boundary conditions, positive supercoiling accumulates between convergently oriented genes and negative supercoiling accumulates between divergently oriented genes. In contrast, with circular boundary conditions, negative supercoiling accumulates in the region between the promoters and positive supercoiling accumulates in the intergenic region in \emph{both} the convergent and divergent syntaxes; the only difference is the relative positioning and inter-element distance of these circuits."
...
\begin{figure}[htbp]
    \centering
    {\includegraphics{figures/modeling_paper/fig_bursting}
    \phantomsubcaption\label{fig:base_model_sc_density} % Plot zero-fold and one-fold induction. Have a supplement figure with all possible inductions.
    \phantomsubcaption\label{fig:burst_dynamics_cartoon}
    \phantomsubcaption\label{fig:base_model_burst_size} % Consider using the 1x induction case
    \phantomsubcaption\label{fig:base_model_interburst_time} % Consider using the 1x induction case
    }
    \caption{Circuit syntax affects both supercoiling density and burst dynamics.
        ...
        \subref{fig:burst_dynamics_cartoon} Given a series of polymerase initiation times, polymerases are grouped into bursts based on a threshold time during which no polymerases are added. Burst size is defined as the number of polymerases in a burst, whereas inter-burst time is defined as the separation in seconds between adjacent bursts. 
        \subref{fig:base_model_burst_size} Burst size distributions are shown as a function of circuit syntax and induction levels of the adjacent gene. The tandem-downstream and convergent syntaxes show a decrease in burst size upon two-fold induction (colored lines) compared to the adjacent uninduced case (gray), whereas the divergent syntax shows an increase in burst size.
        \subref{fig:base_model_interburst_time} Inter-burst time distributions are shown as a function of circuit syntax and induction level. The tandem-upstream and divergent syntaxes show a decrease in the inter-burst time upon adjacent two-fold induction (colored) when compared to the uninduced case (gray).
    }
...
\begin{figure}[htbp]
    ...
    \phantomsubcaption\label{fig:orientation_examples} % Sync gene bar diagram linewidths
    \phantomsubcaption\label{fig:sc_density_turn_on}
...
\begin{figure}[htbp]
    ...
    \caption{Our supercoiling model predicts emergent coupling and noise behavior at a single-cell level.
        \subref{fig:orientation_examples} mRNA counts from example simulation runs in each of the four syntaxes is shown, where transcription of the adjacent gene (gray) is enabled after ten thousand seconds (\(\sim\)3 hrs, shaded background). Both genes have the same base rate, which corresponds to systems with identical promoters and/or promoters with the same expression strength. % TODO: (this needs to describe what the "gene output" is. mRNA counts? Add in A the cartoons of the general understanding of dynamics that we want the reader to take-away from this figure). 
        \subref{fig:sc_density_turn_on} Mean supercoiling density across the simulation ensemble is shown before and after adjacent gene activation.
...
\begin{figure}[htbp]
    ...
    \phantomsubcaption\label{fig:toggle_interburst_time}
    }
\end{figure}
\begin{figure}[htbp]
    \ContinuedFloat
    ...
    \subref{fig:toggle_orientation_examples} Example simulation runs show syntax-dependent stability in toggle switches. Each toggle simulation is started with only one of the genes (colored) active. After 10000 seconds, the second gene is activated.
    ...
    \subref{fig:toggle_interburst_time} Inter-burst time distributions is shown as a function of circuit syntax. Less stable syntaxes show a slight increase in inter-burst time.
...
\begin{figure}[htbp]
    \centering
    {\includegraphics{figures/modeling_paper/fig_toggles_tuning}
    \phantomsubcaption\label{fig:toggle_tunability_cartoon}
    \phantomsubcaption\label{fig:toggle_basin_stability}
    \phantomsubcaption\label{fig:toggle_n1_basins}
    ...
    \caption{
        \subref{fig:toggle_tunability_cartoon} Toggle switch behavior can be tuned by changing circuit syntax, mRNA stability, or by modifying topoisomerase activity.
        ...
        \subref{fig:toggle_n1_basins} The ensemble behavior of \(n = 1.0\) toggle switches is shown. With the exception of the convergently-oriented toggles, the toggle switches reach an equilibrium with a single state without hypersensitivity.
        \subref{fig:toggle_half_life_vs_hill} The half-life, defined as the time it takes half of the simulations in the ensemble to reach the other stability basin, is shown as a function of circuit syntax and Hill coefficient.
